# TASKSURE â€” 20-DAY MVP BUILD PLAN
...
```
Read TASKSURE_CONTEXT.md and TASKSURE_PLAN.md from the project root before doing anything.
TASKSURE_CONTEXT.md has the full product vision, schema, and principles.
TASKSURE_PLAN.md has the day-by-day build plan. Check what day we are on and what is left to build.
All decisions must align with TASKSURE_CONTEXT.md.
```
...
## BEFORE DAY 1 â€” ANTIGRAVITY + MCP SETUP
...
```
PROMPT 1 â€” Sequential Thinking MCP:
"Open the MCP Store in Antigravity (Agent panel â†’ '...' â†’ MCP Servers â†’ MCP Store).
Search for 'Sequential Thinking' and install it. Confirm it is active."
```

```
PROMPT 2 â€” Context7 MCP:
"Open MCP Store â†’ Manage MCP Servers â†’ View raw config (mcp_config.json).
...
I will get my Supabase token from: supabase.com â†’ Account â†’ Access Tokens.
Replace YOUR_SUPABASE_TOKEN with the real token."
```

```
PROMPT 4 â€” Vercel MCP:
...
I will get the token from: vercel.com â†’ Settings â†’ Tokens."
```

```
PROMPT 5 â€” GitHub MCP:
"Add to mcp_config.json:
{
  'github': {
    'command': 'npx',
...
> âš ï¸ **Tool limit:** Antigravity recommends max 50 active tools. Disable Vercel MCP when not deploying. Disable GitHub MCP when not committing. Toggle in Agent panel â†’ '...' â†’ MCP Servers.

---

## PHASE 1 â€” WEB APP FOUNDATION
### Days 1â€“4: Project setup, database, auth, app shell, core pages

---

### DAY 1 â€” Project Setup & Database
**Date:** ___________
...
- [ ] Supabase â€” supabase.com
- [ ] Vercel â€” vercel.com
- [ ] Sarvam AI â€” sarvam.ai (comes with Rs. 1,000 free credit)
- [ ] Google AI Studio â€” aistudio.google.com (get Gemini 2.5 Flash API key)
- [ ] Exotel â€” exotel.com
- [ ] Wati â€” wati.io (start 7-day free trial)
- [ ] GitHub â€” github.com
- [ ] Order new SIM â€” Airtel or Jio website, doorstep delivery, keep unactivated on WhatsApp
...
- [ ] Create all 11 database tables in Supabase with correct schema from TASKSURE_CONTEXT.md
- [ ] Enable RLS on all tables. Write RLS policies.
- [ ] Create all required indexes (listed in TASKSURE_CONTEXT.md schema section)
...
```
PROMPT 1 â€” Project creation:
"Read TASKSURE_CONTEXT.md first. Then:
Using Context7 MCP, look up the official Next.js 14 App Router setup documentation.
Create a new Next.js 14 project called 'TASKSURE' with:
...
```
PROMPT 2 â€” Database tables (run this as one prompt):
"Using the Supabase MCP, create all the following tables in my Supabase project.
Use the exact schema from TASKSURE_CONTEXT.md Section 7.
Create tables in this order (to respect foreign key dependencies):
1. organisations
2. users (self-referential FK on reporting_manager_id â€” create the FK after the table)
3. tasks (parent_task_id self-referential â€” create FK after table)
4. task_comments
5. todos
6. reminders
7. contacts
...
```
PROMPT 3 â€” Indexes:
"Using the Supabase MCP, create all the indexes listed in TASKSURE_CONTEXT.md
under 'Required Database Indexes'. Run them now."
```

```
PROMPT 4 â€” Supabase client:
...
```
PROMPT 5 â€” GitHub:
"Using GitHub MCP, create a private repository called 'TASKSURE'.
...
### DAY 2 â€” Authentication & Onboarding Flow
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
#### Tasks
- [ ] Build /login page â€” phone number input, sends OTP via Supabase Auth
- [ ] Build /login/verify page â€” 6-digit OTP entry and verification
- [ ] Build /signup page â€” collect name, org (create new or join existing), reporting manager name
- [ ] Org logic: check if org name exists â†’ join it; if not â†’ create it
- [ ] Reporting manager logic: look up by name in same org â†’ link if found; flag as pending if not found
- [ ] Build auth middleware â€” redirects unauthenticated users from any /dashboard/* route to /login
...
3. app/signup/page.tsx
   ...
   - Reporting Manager Name field (optional â€” with note: 'Leave blank if you are the owner')
   - Submit button
...
### DAY 3 â€” App Shell & Home Dashboard
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- Notification bell shows a red badge with unread count (hardcode as 0 for now â€” we will wire it up later)
...
1. GREETING ROW
   'Good morning, [First Name]' â€” dynamic based on time of day (morning/afternoon/evening)
...
Use Tailwind for all styling. Use Next.js server components â€” fetch data on the server.
If any section has no items, show a friendly empty state (e.g. 'No pending tasks today ðŸŽ‰')."
```

```
PROMPT 3 â€” Mobile test:
...
### DAY 4 â€” My Tasks, Assigned Tasks & Calendar
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- If sub-tasks are not all done: show error message 'Cannot complete â€” sub-task assigned to [name] is still pending'
...
```
PROMPT 2 â€” Assigned Tasks page:
...
- Original deadline
- Committed deadline (shown only if accepted â€” 'Committed to: 5 Mar 5pm')
- Status badge
- 'View Details' button
...
## PHASE 2 â€” WEB APP ADVANCED FEATURES
### Days 5â€“8: Team, Stats, Todos, Real-time, Notifications, Create Task

---

### DAY 5 â€” Team, Stats & Settings Pages
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
TAB 2 â€” Organisation (only visible if current user role = 'owner'):
...
### DAY 6 â€” Todos Page, Create Task, Real-Time & Notifications
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
#### Tasks
- [ ] Build /todos page â€” private to current user, no one else can access
...
- [ ] Add Supabase real-time subscriptions to dashboard â€” status changes update all tabs instantly
...
- 'When do you want to be reminded?' â€” date + time picker â†’ saves to remind_at
- 'What is the due date?' â€” date picker (defaults to today) â†’ saves to due_at
...
- Assign To (required â€” autocomplete input):
...
- Deadline (required â€” date + time picker)
...
```
PROMPT 3 â€” Real-time subscriptions:
...
#### End of Day Check
- [ ] Todos page shows only my todos â€” cannot see anyone else's
- [ ] Creating a todo inserts reminder rows in Supabase correctly
- [ ] Create Task modal works end-to-end â€” task appears in assignee's My Tasks
- [ ] Opening two browser windows â€” status change in one updates the other in real-time
- [ ] Notification bell count updates in real-time

---

### DAY 7 â€” Web App Polish, Mobile & Deploy
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
**Goal:** Web app is pixel-perfect on mobile. Deployed to Vercel production. Full end-to-end browser test passing.

#### Tasks
- [ ] Full mobile audit of every page (375px viewport) â€” fix all issues
...
- [ ] Buffer time â€” catch up on anything incomplete from Days 1â€“6

#### Antigravity Prompts

```
PROMPT 1 â€” Mobile audit:
...
```
PROMPT 2 â€” End-to-end test:
"Open the Browser Agent and test the complete web app flow as a new user.
Do NOT use existing test data â€” simulate a truly fresh signup.

Test flow:
1. Go to /login â†’ enter a test phone number â†’ verify OTP
2. Complete /signup â†’ create org 'Test Company' â†’ set no reporting manager (owner)
3. Land on /dashboard/home â†’ verify greeting and empty states
4. Create a task assigned to test user 2 (first invite them if needed)
5. Switch to test user 2's session â†’ see task in Pending Acceptance
6. Accept the task with a deadline 1 hour from now
7. Switch back to user 1 â†’ verify real-time update shows task accepted
8. Switch back to user 2 â†’ mark task as DONE
9. Verify user 1 gets a notification
10. Check /stats â†’ verify task appears in completed count
11. Add a personal todo â†’ verify it's private and appears on /todos

Screenshot every step. Give me a final verdict: pass or fail per step."
```

```
PROMPT 3 â€” Vercel deploy:
"Using Vercel MCP, deploy my TASKSURE project to production.
Set these environment variables in Vercel:
- NEXT_PUBLIC_SUPABASE_URL (from Supabase project settings)
- NEXT_PUBLIC_SUPABASE_ANON_KEY (from Supabase project settings)
- SUPABASE_SERVICE_ROLE_KEY (from Supabase project settings â†’ KEEP THIS SERVER-SIDE ONLY)
...
## PHASE 3 â€” WHATSAPP BOT
### Days 8â€“13: Webhook, transcription, intent classification, task flows, todo flows, reminder system

---

### DAY 8 â€” Webhook Setup & Voice Transcription
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
**Goal:** WhatsApp messages (text and voice) arrive in our system and are logged. Voice notes transcribed accurately in Gujarati.

> âš ï¸ Your new SIM should have arrived by now. Activate it and register it with Wati before starting this day.
...
- [ ] Deploy webhook URL to Vercel, paste into Wati â†’ Settings â†’ Webhook
- [ ] Verify test messages arrive in Vercel logs
- [ ] Build lib/transcribe.ts using Sarvam Saaras v3 with Gujarati + code_mix support
- [ ] Test with 5 Gujarati voice notes â€” verify accurate transcripts
...
- If message type is 'text': proceed directly to intent classification (lib/classifyIntent.ts â€” we build this tomorrow)

Add a WATI_VERIFY_TOKEN environment variable with value 'TASKSURE_webhook_2025'"
```

```
PROMPT 2 â€” Voice transcription:
...
### DAY 9 â€” Intent Classification & Task Creation Flow
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- [ ] Build lib/extractTask.ts â€” extracts assignee, description, deadline from task_create messages
- [ ] Build lib/processTaskCreation.ts â€” full orchestration: lookup user â†’ create task â†’ send WhatsApp â†’ insert reminders
...
```
PROMPT 1 â€” Intent classification:
...
Examples:
Input: Ramesh ne file 5 vaage moklavani chhe â†’ {intent: task_create, confidence: high}
Input: Remind me to call Mehta at 3pm â†’ {intent: todo_create, confidence: high}
Input: ACCEPT â†’ {intent: task_reply, confidence: high, raw_reply_keyword: ACCEPT}
Input: Flowers lava javu chhe aaje â†’ {intent: todo_create, confidence: high}
Input: hello â†’ {intent: unknown, confidence: high}
...
```
PROMPT 3 â€” Task creation orchestration:
...
Logic:
1. Look up sender in users table by phone_number â†’ get their user ID and organisation_id
   If sender not in users table: send WhatsApp 'You are not registered in TASKSURE. Please sign up at [URL]' and return.
...
5. If deadline_iso is null:
   ...
   (For now, skip the session handling â€” just ask for deadline and let them re-send the full message with deadline)
...
'âœ… Task assigned to [Assignee Name]!
...
### DAY 10 â€” Task Reply Flows & Todo Creation
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- [ ] ACCEPT flow: ask for deadline â†’ parse response â†’ update task â†’ notify manager
- [ ] REJECT flow: ask for reason â†’ save to task_comments â†’ update task â†’ notify manager
- [ ] DONE flow: check sub-tasks complete â†’ mark done â†’ notify manager
- [ ] DELEGATE flow: parse delegation target and sub-task â†’ create sub-task â†’ notify delegate
- [ ] HELP command: list all commands
- [ ] STATUS command: show user's active tasks
- [ ] Build lib/processTodoCreation.ts â€” creates todo and reminder rows from WhatsApp
- [ ] Test all commands end-to-end

#### Antigravity Prompts

```
PROMPT 1 â€” Task reply handler:
...
- Reply to assignee: 'Perfect! Task accepted. Deadline set to [formatted deadline]. Good luck! ðŸ’ª'
...
- If sub-tasks incomplete: reply 'Cannot complete yet â€” [sub-assignee name]'s sub-task is still pending.'
...
- Reply to sender: 'ðŸŽ‰ Task marked as complete!'
- Reply to task creator: 'âœ… [Name] has completed the task: [task title]'

HELP:
- Reply with formatted list:
'TASKSURE Commands:
âœ… *ACCEPT* â€” Accept a task assigned to you
âŒ *REJECT* â€” Reject a task (will ask for reason)
âœ… *DONE* â€” Mark your task as complete
ðŸ“¤ *DELEGATE* â€” Delegate to a team member (e.g. DELEGATE to Priya: get invoice by 3pm)
ðŸ“‹ *STATUS* â€” See your active tasks
...
- Reply with formatted list:
'Your active tasks:
1. [task title] â€” [status] â€” Due: [deadline]
2. [task title] â€” [status] â€” Due: [deadline]
(none if empty)'
...
```
PROMPT 2 â€” Todo creation via WhatsApp:
...
```
PROMPT 3 â€” Wire everything into webhook:
...
Route to:
- task_create â†’ processTaskCreation()
- todo_create â†’ processTodoCreation()
- task_reply â†’ processTaskReply()
- unknown â†’ sendWhatsApp(phone, 'I did not understand that. Reply HELP for a list of commands.')
...
Use Promise callbacks or a background queue pattern â€” do not await in the main handler."
```

#### End of Day Check
- [ ] ACCEPT â†’ deadline prompt â†’ deadline reply â†’ task accepted â†’ manager notified
- [ ] REJECT â†’ reason prompt â†’ reason reply â†’ task rejected â†’ manager notified
- [ ] DONE â†’ task completed â†’ manager notified
- [ ] DELEGATE â†’ sub-task created â†’ delegate notified â†’ ACCEPT blocks parent
- [ ] "Remind me to call Mehta at 3pm" â†’ todo created â†’ reminder row inserted in Supabase

---

### DAY 11 â€” Reminder System (Cron Job)
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
#### Tasks
- [ ] Build /api/cron/reminders route â€” the main cron job
...
```
PROMPT 1 â€” Voice call function:
...
STEP 1 â€” Process pending reminders:
...
- Task not accepted (10 min): 'Hi [name] ðŸ‘‹ You have a task waiting for your response from [manager]. Reply ACCEPT or REJECT on this WhatsApp.'
- Task deadline approaching (1 hr): 'âš ï¸ Reminder: Your task "[title]" is due at [time]. Please complete it on time!'
- Todo reminder: 'ðŸ“ Reminder: [todo title]'
- Todo EOD: 'ðŸŒ™ End of day reminder: [todo title] â€” was this completed? Reply DONE if yes.'
...
For each:
  ...
  - Send WhatsApp to assigned_to: 'ðŸš¨ Your task "[title]" is now overdue! Please complete it immediately or contact [manager name].'
  - Send WhatsApp to created_by: 'ðŸš¨ OVERDUE: [assignee name] has not completed "[title]" by [committed_deadline].'
  - INSERT audit_log: { action:'task.overdue' }

STEP 3 â€” Log job run:
INSERT audit_log: { action:'system.cron_run', metadata: { reminders_processed: N, tasks_marked_overdue: M } }"
```

```
PROMPT 3 â€” Vercel Cron setup:
...
#### End of Day Check
- [ ] Create a task, wait 10 minutes â€” WhatsApp reminder received
- [ ] Wait 30 minutes total â€” voice call received (check Exotel call logs)
- [ ] Set a task deadline 1 minute in the past â€” it appears as overdue within 5 minutes
- [ ] Complete a task â€” its pending reminders are cancelled in Supabase

---

### DAY 12 â€” Gujarati Testing & WhatsApp Polish
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
**Goal:** 30 test messages processed correctly. All bot messages rewritten to sound natural. All commands working.

#### Tasks
- [ ] Test 30 diverse messages across all intent types â€” document pass/fail
...
```
PROMPT 1 â€” 30-message test suite:
...
```
PROMPT 2 â€” Fix failures and polish messages:
...
- Use appropriate emoji (not too many â€” 1-2 per message max)
...
- Keep messages concise â€” max 3-4 lines
...
â€¢ Ramesh: 2 pending, 1 overdue âš ï¸
â€¢ Priya: 3 accepted, due today: 1
â€¢ Suresh: 1 pending (not accepted yet)'

WELCOME MESSAGE (for any new phone number messaging the bot for first time):
- Check if incoming phone exists in users table
- If not found: reply with welcome message introducing TASKSURE and the signup URL
'Hello! ðŸ‘‹ I'm TASKSURE â€” your team's task management assistant.
...
## PHASE 4 â€” SECURITY, TESTING & LAUNCH
### Days 13â€“20: Security hardening, beta testing, bug fixes, launch

---

### DAY 13 â€” Security Hardening & Full System Audit
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- [ ] Run full Snyk security scan â€” fix all HIGH severity issues
...
```
PROMPT 1 â€” RLS verification:
"Using Supabase MCP, run these security tests:

TEST 1 â€” Cross-org task isolation:
...
TEST 3 â€” Auth route protection:
...
TEST 4 â€” Service role key exposure:
...
```
PROMPT 3 â€” GitHub commit:
"Using GitHub MCP, commit all security fixes:
'Day 13: Security hardening â€” RLS verified, Snyk clean, rate limiting active'
Tag as v0.3.0-security"
```

#### End of Day Check
- [ ] RLS tests pass â€” zero cross-org data leakage
- [ ] Todo privacy test passes â€” zero cross-user todo visibility
...
### DAY 14 â€” Performance & Final Pre-Beta Polish
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
**Goal:** All pages load under 2 seconds. All edge cases handled. App ready for real users.

#### Tasks
- [ ] Measure home dashboard load time â€” optimise if over 2 seconds
...
- [ ] Final full mobile test â€” fix any remaining issues
- [ ] Test on Safari mobile (iOS) specifically â€” often has different behaviour
- [ ] Make sure the web app's Create Task flow and WhatsApp bot stay in sync (same data)

#### Antigravity Prompts

```
PROMPT 1 â€” Performance audit:
"Using Browser Agent and the Vercel MCP, run a performance audit.

Check 1 â€” Page load times:
...
Check 3 â€” Cron job performance:
...
```
PROMPT 2 â€” Polish and error pages:
...
- [ ] Do NOT fix anything today â€” only observe and document

#### Antigravity Prompts

```
PROMPT 1 â€” Pre-beta monitoring setup:
...
```
PROMPT 2 â€” Onboarding guide creation:
...
#### Day 15 Evening â€” Document Findings
After beta testing:
- [ ] List all bugs found
...
### DAY 16 â€” Beta Bug Fixes (Round 1)
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
```
PROMPT 1 â€” Flexible deadline parsing:
...
### DAY 17 â€” Beta Bug Fixes (Round 2) & Monitoring
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- [ ] Buffer day â€” use remaining time to address any leftover items from earlier days

#### Antigravity Prompts

```
PROMPT 1 â€” System health check:
...
### DAY 18 â€” Landing Page & Onboarding UX
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- [ ] Record a 3-minute Loom demo video (loom.com â€” free)
- [ ] Write 1-page onboarding guide (WhatsApp-forward-friendly format)
- [ ] Update TASKSURE_PLAN.md with current build status

#### Antigravity Prompts

```
PROMPT 1 â€” Landing page:
...
Sub-headline: 'TASKSURE turns your WhatsApp voice notes into tracked tasks with automatic follow-ups â€” built for Indian businesses.'
CTA button: 'Start Free' â†’ links to /signup
Small trust note: 'Works in Gujarati, Hindi, and English'

HOW IT WORKS (3 steps with icons):
1. ðŸŽ¤ Send a voice note â€” Tell the bot who should do what and by when
2. âœ… Employee accepts â€” They commit to a deadline on WhatsApp
3. ðŸ“Š You track everything â€” Dashboard shows all tasks, deadlines, and performance

KEY FEATURES (3 feature cards):
- Voice-first in Gujarati/Hindi â€” No typing required
- Automatic follow-ups â€” Calls and WhatsApp reminders without you lifting a finger
- Performance analytics â€” Know who delivers and who delays
...
- No external images â€” use emoji and CSS for visual elements
- Page must load in under 1 second (no heavy assets)"
```

```
PROMPT 2 â€” Update plan file:
"Update the Current Build Status section in TASKSURE_PLAN.md.
...
### DAY 19 â€” Final QA & Pre-Launch Checklist
**Date:** ___________
**Status:** `[ ] Not started` â†’ `[ ] In progress` â†’ `[ ] Complete`
...
- [ ] Final Snyk security scan â€” must be clean
...
```
PROMPT 1 â€” Complete end-to-end QA:
...
FLOW 2 â€” WhatsApp Voice Task (simulate with text):
...
FLOW 3 â€” Personal Todo:
1. Ramesh sends WhatsApp: 'Remind me to call the supplier at 11am tomorrow'
2. Todo is created (verify in Supabase todos table â€” only visible to Ramesh)
3. Ramesh opens /todos â€” sees his todo (private)
4. Vikram opens /todos â€” does NOT see Ramesh's todo
PASS or FAIL each step.

FLOW 4 â€” Reminder Escalation:
...
```
PROMPT 2 â€” Pre-launch checklist:
"Run through this pre-launch checklist and verify each item:

SECURITY:
[ ] RLS enabled on all 11 tables (check in Supabase â†’ Authentication â†’ Policies)
[ ] SUPABASE_SERVICE_ROLE_KEY not in any client-side code (search codebase)
[ ] All environment variables set in Vercel (check Vercel â†’ Settings â†’ Environment Variables)
...
- [ ] Update TASKSURE_CONTEXT.md: update build status, live URL, user count
- [ ] Update TASKSURE_PLAN.md: mark all items complete
...
Using GitHub MCP:
1. Commit any last changes with message 'Day 20: MVP Launch â€” TASKSURE v1.0.0'
2. Push to main branch"
```

```
PROMPT 2 â€” Update context files:
"Update TASKSURE_CONTEXT.md Section 12 (Current Build Status):
- Mark all items as [x] complete
- Set Current version: v1.0.0-mvp (launched)
- Set Live URL: [actual URL]
- Set Beta users: [actual count]

Update TASKSURE_PLAN.md:
- Add a new section at the bottom: ## POST-LAUNCH â€” WHAT'S NEXT
...
```
PROMPT 3 â€” Post-MVP planning prompt (save this for future use):
"[Save this prompt for your first post-MVP Antigravity session]

Read TASKSURE_CONTEXT.md and TASKSURE_PLAN.md.
...
1. Confirm the feature aligns with the Core Product Principles in TASKSURE_CONTEXT.md
2. Confirm it is not in the 'What We Are NOT Building' section
3. Check if the database schema already supports it (it likely does â€” schema was built for scale)
...
WATI_API_KEY=
WATI_API_URL=
WATI_VERIFY_TOKEN=TASKSURE_webhook_2025
...
## APPENDIX B â€” FILE STRUCTURE REFERENCE
...
â”‚   â”œâ”€â”€ login/
â”‚   â”‚   â”œâ”€â”€ page.tsx                    # Phone number entry
â”‚   â”‚   â””â”€â”€ verify/page.tsx             # OTP verification
â”‚   â”œâ”€â”€ signup/
â”‚   â”‚   â””â”€â”€ page.tsx                    # Org + manager setup
â”‚   â”œâ”€â”€ auth/
...
â”‚   â”‚   â”œâ”€â”€ my-tasks/page.tsx           # My Tasks
â”‚   â”‚   â”œâ”€â”€ assigned-tasks/page.tsx     # Tasks I Assigned
â”‚   â”‚   â”œâ”€â”€ todos/page.tsx              # Personal To-Dos (private)
â”‚   â”‚   â”œâ”€â”€ calendar/page.tsx           # Calendar
â”‚   â”‚   â”œâ”€â”€ team/page.tsx               # Org Chart
â”‚   â”‚   â”œâ”€â”€ stats/page.tsx              # Performance Stats
â”‚   â”‚   â”œâ”€â”€ notifications/page.tsx      # Notifications
â”‚   â”‚   â””â”€â”€ settings/page.tsx           # Settings
â”‚   â””â”€â”€ api/
â”‚       â”œâ”€â”€ webhook/route.ts            # WhatsApp webhook (Wati)
â”‚       â””â”€â”€ cron/
â”‚           â””â”€â”€ reminders/route.ts      # Reminder cron job (every 5 min)
â”œâ”€â”€ lib/
â”‚   â”œâ”€â”€ supabase/
â”‚   â”‚   â”œâ”€â”€ client.ts                   # Browser Supabase client
â”‚   â”‚   â””â”€â”€ server.ts                   # Server Supabase client
â”‚   â”œâ”€â”€ transcribe.ts                   # Sarvam Saaras v3 STT
â”‚   â”œâ”€â”€ classifyIntent.ts               # Gemini 2.5 Flash intent classification
â”‚   â”œâ”€â”€ extractTask.ts                  # Gemini 2.5 Flash task extraction
â”‚   â”œâ”€â”€ sendWhatsApp.ts                 # Wati WhatsApp message sender
â”‚   â”œâ”€â”€ makeVoiceCall.ts                # Sarvam Samvaad + Exotel outbound call
â”‚   â”œâ”€â”€ processTaskCreation.ts          # Task creation orchestration
â”‚   â”œâ”€â”€ processTaskReply.ts             # ACCEPT/REJECT/DONE/DELEGATE handler
â”‚   â””â”€â”€ processTodoCreation.ts          # Todo creation orchestration
â”œâ”€â”€ providers/
â”‚   â””â”€â”€ RealtimeProvider.tsx            # Supabase real-time context
â”œâ”€â”€ middleware.ts                       # Auth route protection
â”œâ”€â”€ vercel.json                         # Cron job configuration
â”œâ”€â”€ TASKSURE_CONTEXT.md                 # Project vision & schema (source of truth)
â”œâ”€â”€ TASKSURE_PLAN.md                    # This file â€” build plan
â””â”€â”€ .env.local                          # Environment variables (never commit)
```

---

## APPENDIX C â€” IF YOU FALL BEHIND
...
**Fell behind by 2-3 days:** Combine Days 15+16 (beta setup + first round of fixes) into one day. Combine Days 17+18 into one day. Cut the landing page (Day 18) â€” you can add it after launch.

**Fell behind by 4+ days:** Cut Day 14 (performance optimisation) â€” it can be done post-launch. Cut Day 19 (full QA) â€” do a condensed 30-minute manual check instead. Launch with known minor issues and fix post-launch. Shipping is more important than perfection at MVP stage.
